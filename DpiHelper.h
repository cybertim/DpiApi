...
public:
	...
	*/
	...
	DpiHelper();
	~DpiHelper();
	static std::map<int, DpiHelper::DisplayData> m_displayDataCache;
	...
	static std::map<int, DisplayData> RefreshDpiInfo();
	static INT32 GetIDHigh(INT32 index);
	static INT32 GetIDLow(INT32 index);
	static INT32 GetIDSource(INT32 index);
	static INT32 GetIndexCount();
};